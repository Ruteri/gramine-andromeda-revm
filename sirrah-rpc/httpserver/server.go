...
import (
	...
	"github.com/flashbots/gramine-andromeda-revm/sirrah-rpc/common"
...
func New(cfg *HTTPServerConfig) (srv *Server, err error) {
	...
	revmService, err := NewRevmService(cfg.Log, metricsSrv)
	if err != nil {
		return nil, err
	}
	...
	mux := chi.NewRouter()
	mux.With(srv.httpLogger).Get("/rpc", srv.handleAPI) // Never serve at `/` (root) path
